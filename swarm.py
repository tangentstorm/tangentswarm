...
def get_args():
    ...
    if len(args) == 1:
        ...
        if not config:
            ...
            sys.exit(1)
        default_repo = list(config.keys())[0]
        return default_repo, args[0]
    elif len(args) == 2:
...
def main():
    # Load arguments
    repo_name, branch_name = get_args()
    
    # Load config
    config = load_config()
    
    # Ensure repo exists in config
    if repo_name not in config:
        config[repo_name] = {}
    
    # Ensure branch exists in repo config
    if branch_name not in config[repo_name]:
        ...
        config[repo_name][branch_name] = port
        save_config(config)
    ...
    if not os.path.exists(branch_dir):
        ...
        subprocess.run(['git', 'clone', repo_name, branch_dir], check=True)
